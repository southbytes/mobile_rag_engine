...
''',
    ...
    for (var i = 0; i < documents.length; i++) {
      ...
      _log(
        '  Source ID: ${result.sourceId}, Chunks: ${result.chunkCount}, Duplicate: ${result.isDuplicate}',
      );
    }
  ...
  Future<void> _testSearch() async {
    ...
    final queries = ['smartphone company', 'search engine', 'operating system'];

    for (final query in queries) {
      _log('Query: "$query"');

      final result = await _rag!.search(query, topK: 3, tokenBudget: 1000);

      _log('  Found ${result.chunks.length} chunks');
      for (var i = 0; i < result.chunks.length && i < 2; i++) {
        final chunk = result.chunks[i];
        final preview = chunk.content.substring(
          0,
          chunk.content.length.clamp(0, 50),
        );
        _log(
          '  ${i + 1}. [sim: ${chunk.similarity.toStringAsFixed(3)}] "$preview..."',
        );
      }
    }
  ...
  Future<void> _testContextBuilder() async {
    ...
    final prompt = _rag!.formatPrompt(
    ...
    assert(
      result.context.estimatedTokens <= 500,
